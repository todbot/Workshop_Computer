...
extern "C" uint8_t g_sampleRateIdx;
...
void __not_in_flash_func(ComputerCard::AudioWorker)()
{
    ...
    // 48MHz ÷ (124+1) = 384kHz ADC sample rate = 8 * 48kHz
    // 48MHz ÷ (135.0544+1) = 352.8kHz ADC sample rate = 8 * 44.1kHz
    // 48MHz ÷ (249+1) = 192kHz ADC sample rate = 8 * 24kHz

    float clkDiv = 124.0f; // Default 48k
    if (g_sampleRateIdx == 1) clkDiv = 135.0544f; // 44.1k (Precise)
    else if (g_sampleRateIdx == 2) clkDiv = 249.0f; // 24k
